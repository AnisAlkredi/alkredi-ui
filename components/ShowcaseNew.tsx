...
function PillSmall({ text }: { text: string }) {
  return (
...
function ColorDot({ color }: { color: string }) {
  return (
    <span
      className="h-7 w-7 rounded-full border-2 border-white shadow-[0_10px_28px_rgba(15,23,42,0.14)]"
...
/** رابط “Details” يذهب إلى فورم التواصل */
function DetailsLink({ lang }: { lang: Lang }) {
  return (
    <a
      href="#contact"
      className="inline-flex items-center text-sm font-semibold text-[#0C8F7D] opacity-90 transition hover:opacity-100"
    >
      {t(lang, "showcase.cta")} →
    </a>
  );
...
function MiniAppMock({
  ...
  // بلوك النص الذي سيُعرض داخل الشاشة
  const InsideText = (
    <div className="mt-4 rounded-xl border border-slate-200/60 bg-white/70 p-3">
      <div className="flex items-start justify-between gap-2">
        ...
        {badge ? <PillSmall text={badge} /> : null}
      </div>

      <div className="mt-2 flex items-center gap-2">
        <DotStack colors={dotColors} />
        <span className="text-[11px] text-slate-600">{contributorsLabel(lang)}</span>
      </div>

      <p className="mt-2 text-[12px] leading-relaxed text-slate-700 line-clamp-3">
        {description}
      </p>

      {/* Details داخل البطاقة */}
      <div className="mt-3">
        <DetailsLink lang={lang} />
      </div>
    </div>
  );
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {items.slice(0, 3).map((it, idx) => (
            ...
            >
              <MiniAppMock
                variant={variants[idx] ?? "web"}
                title={it.title}
                description={it.description}
                badge={it.badge}
                dotColors={dotColors[idx] ?? dotColors[0]}
                lang={lang}
              />

              {/* CTA خارجي إضافي (اختياري) — يذهب أيضًا إلى فورم التواصل */}
              <div className="mt-4">
                <a
                  href="#contact"
                  className="text-sm font-semibold text-[#0C8F7D] opacity-90 transition hover:opacity-100"
                >
                  {t(lang, "showcase.cta")} →
                </a>
              </div>
